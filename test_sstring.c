...
#define TEST_IS_INT(val)                                                       \
  {                                                                            \
    sstring ss = sstring_create_string(#val);                                  \
    int n;                                                                     \
    if ((!sstring_is_integer(ss, &n)) || (n != val)) {                         \
      puts("TEST TEST_IS_INT fail for " #val);                                 \
    }                                                                          \
    sstring_destroy(&ss);                                                      \
  }

#define TEST_IS_NOT_INT(str)                                                   \
  {                                                                            \
    sstring ss = sstring_create_string(str);                                   \
    int n;                                                                     \
    if (sstring_is_integer(ss, &n)) {                                          \
      puts("TEST TEST_IS_INT fail for " str);                                  \
    }                                                                          \
    sstring_destroy(&ss);                                                      \
  }
 ...
 * \param ss \c sstring to print
 */
static void test_sstring_println_st_ss(char const *const st, sstring ss) {
  assert(NULL != st);
  assert(NULL != ss);
  puts("---------------");
 ...
 */
static void test_sstring_compare(char const *const st1, char const *const st2) {
  assert(NULL != st1);
  assert(NULL != st2);
...
static void test_sstring(char const *const st1, char const *const st2) {
  ...
  // length
  int const length_ss1 = sstring_get_length(ss1);
  fprintf(stdout, "length ( ss1 ) = %d\n", length_ss1);
  // first and ultimate characters
  if (0 < length_ss1) {
    fprintf(stdout, "first and last char of ( ss1 ) = '%c' and '%c'\n",
            sstring_get_char(ss1, 0), sstring_get_char(ss1, length_ss1 - 1));
  }
...
/*!
 * Launch \link\c test_sstring_test() \enlink on various cases including empty
 * C-string.
 */
int main(void) {
